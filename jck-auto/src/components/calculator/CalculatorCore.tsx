...
/**
 * @file CalculatorCore.tsx
 * @description Shared calculator body — form, calculation, result. Used by /tools/calculator page and the homepage section. Single source of truth for calculator UI and logic.
 * @runs Next.js client (browser)
 * @rule This is the ONLY place that contains calculator form state and result rendering. /tools/calculator/CalculatorClient.tsx and components/sections/Calculator.tsx are thin wrappers around this — they only provide page chrome.
 * @rule Currency rates come from /api/exchange-rates (server route). Do NOT import fetchCBRRates here — it would re-introduce the browser CORS issue.
 * @lastModified 2026-04-08
 */

import { useState, useEffect } from "react";
import Link from "next/link";
import { Calculator as CalcIcon, Check, Phone, Loader2, Send, ArrowRight } from "lucide-react";
...
export default function CalculatorCore({ showDeepLink = false }: CalculatorCoreProps) {
      ...
      <div className="flex flex-col justify-center">
        {result ? (
          <div>
            ...
            {showDeepLink && (
              <Link href="/tools/calculator" className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
                Открыть полный калькулятор <ArrowRight className="h-4 w-4" />
              </Link>
            )}
          </div>
        ) : (
