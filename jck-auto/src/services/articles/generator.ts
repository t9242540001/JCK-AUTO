...
ФОРМАТ СТАТЬИ:
- Длина: 2000-2500 слов (минимум 1800)
...
${topic.internalLinks.map((l) => `- ${l}`).join('\n')}`;
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    const extraInstruction = attempt > 0
      ? '\n\nВАЖНО: предыдущий ответ был слишком коротким. Напиши минимум 2000 слов. Раскрой каждый раздел подробнее, добавь примеры, таблицы, конкретные цифры.'
      : '';
