...
import { sendEncarInstructions } from '../lib/instructionMessages';
...
export function registerEncarHandler(bot: TelegramBot): void {
  // /encar slash-command — instruction message (shared with encar_info callback in start.ts)
  bot.onText(/^\/encar\b/, async (msg) => {
    sendEncarInstructions(bot, msg.chat.id);
  });
