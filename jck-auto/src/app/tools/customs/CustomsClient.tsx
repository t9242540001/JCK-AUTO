...
const pricePlaceholder: Record<CurrencyCode, string> = {
  CNY: "Например 82000",
  KRW: "Например 35000000",
  JPY: "Например 2500000",
  EUR: "Например 15000",
  USD: "Например 18000",
};
        ...
        {result.breakdown.map((item, i) => (
          <div key={i} className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              {item.details && (
                <p className="mt-0.5 text-xs italic text-text-muted/70">{item.details}</p>
              )}
            </div>
...
export default function CustomsClient() {
  ...
  const handleCalculate = () => {
    ...
    const base = { priceInCurrency: p, currencyCode, engineVolume: v, enginePower: hp, carAge: age };
        ...
        {!rates ? (
          ...
          <>
            ...
            <div className="grid gap-5 md:grid-cols-3">
              ...
              <div>
                <label className="text-sm font-medium text-text">Мощность (л.с.)</label>
                <input type="number" value={power} onChange={(e) => { setPower(e.target.value); clearResults(); }} placeholder="Например 150" className={inputClass} />
              </div>
              <div>
