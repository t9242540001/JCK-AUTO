...
- Сайт: `pm2 restart jckauto --update-env` после `ln -sfn` symlink swap, **не `pm2 startOrReload`** — `startOrReload` для уже-online процесса = graceful reload, который не сбрасывает in-memory Next.js chunks из старого слота. Симптом без правильного restart: цикл ENOENT-падений ~каждые 94 секунды с stack-trace вида `at async Module.N (.next-b/server/...)` (in-memory код старого слота читает BUILD_ID нового слота, не сходится). См. ADR `[2026-04-28] Б-7 closed`.
