<!--
  @file:        knowledge/bugs-archive-1.md
  @project:     JCK AUTO
  @description: Archive 1 of bugs.md — 11 closed bug entries from 2026-03..2026-04-26
  @updated:     2026-05-02
  @version:     1.0
  @lines:       ~280
  @note:        Read-only archive. NOT read at session start by default. Open
                only when investigating historical bug context — when a
                current bug or ADR references a closed bug by ID, or when
                debugging requires understanding past failure modes. Created
                by KC-3 (knowledge-cleanup series, 2026-05-02). Cutoff:
                bugs with Status:Closed move here; OPEN, VERIFY, and WON'T-FIX
                entries remain in active bugs.md. Bug IDs preserved verbatim
                (С-1, С-2, С-8, Б-4, Б-6, Б-9, Б-11, Б-12, Б-13, Б-14, Б-15).
-->

# Bugs — Archive 1

> Closed bug entries from 2026-03..2026-04-26. For active bugs (open,
> verify-status, won't-fix), see `bugs.md`. Each entry preserves its full
> context including Symptom, Root cause, Fix, and any Status Updates.
> Section structure (Critical / Important / Verify status) preserved
> from original.
