...
|------|-------------|---------|
| [infrastructure.md](infrastructure.md) | Server identity, PM2 processes (driven by committed ecosystem.config.js, logs at /var/log/pm2/ per INFRA-1), cron jobs (committed crontab via scripts/crontab.root + scripts/install-crontab.sh per INFRA-1.5), deploy pipeline, runtime constraints, CI/CD, active bugs. Network config (nginx, Cloudflare, Yandex MCP install) → networking.md | 2026-05-04 |
...
| [decisions.md](decisions.md) | Architectural Decision Records (ADR log) — active section. Contains 16 most recent ADRs (2026-04-29 onward) + CRIT-1 + SALES-CRIT-2 + INFRA-1 + INFRA-1.5 (2026-05-04). Older 81 entries → [decisions-archive-1.md](decisions-archive-1.md). Auto-archive trigger at 1000 lines. | 2026-05-04 |
