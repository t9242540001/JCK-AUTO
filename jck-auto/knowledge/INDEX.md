...
|------|-------------|---------|
| [infrastructure.md](infrastructure.md) | Server, PM2 (3 processes), deploy commands, nginx (default 60s + per-endpoint overrides for /api/tools/auction-sheet: 200s timeout + 15MB body), Cloudflare Worker tg-proxy (4 routing modes), VDS provider network restrictions + updated bot canonical startup command with cd + bash -c wrapper + --max-restarts 5 (post-incident 2026-04-22) | 2026-04-22 |
...
| [bugs.md](bugs.md) | Open bugs tracker — Б-9 closed (user store race on bot restart), С-1 closed, С-5 user-impact closed (moved to Verify), С-6 closed (cross-tab session leak fixed), С-4 closed (/api/lead hardened), С-3 closed (on-primary CTA variant), Б-1 closed (Cloudflare Smart Placement), Б-2 and Б-3 closed (side-effect of Smart Placement), С-2 open, Б-4, Б-5, Б-6, Б-7, Б-8 open, С-8 open (encar handler timeout). Middleware-manifest restart loop (Б-7), Capture workflow verification (Б-8) pending | 2026-04-22 |
